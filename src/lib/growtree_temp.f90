...
!##################################################
  ...
  subroutine smooth_1d_tree(num_elem_start,length_limit)
    ...
    use indices
    use mesh_utilities,only: distance_between_points, unit_vector ! (MS) added: distance_between_points
    ...
    do n = 1,n_smoothing_steps
       do ne = num_elems,num_elem_start,-1
          if(elem_cnct(1,0,ne).eq.2)then
             ...
             ! (MS) added: update the direction and length of the element
             DIRECTION(1:3)=(node_xyz(1:3,np)-node_xyz(1:3,np0))
             DIRECTION = unit_vector(DIRECTION)
             elem_direction(1:3,ne) = DIRECTION(1:3)
