...
!!!#############################################################################
  ...
  subroutine evaluate_vent
    ...
    integer :: kount,n_steps !(MS) added
...
!!! calculate the total model volume
   ...
   write (*,*) 'The order of the tranchea is', elem_ordrs(no_Hord,1)
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! (MS) initialise array to store unit ppl per timestep !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    n_steps = ceiling(T_interval/dt)+1 ! no. of time steps in a breath = T_interval/dt, plus extra row jic
    !print*, "Num of time steps in a breath:", n_steps ! this is ok
    allocate(unit_ppl(1:n_steps,1:num_units))
    unit_ppl(1:n_steps,1:num_units) = 0.0_dp
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! (MS) initialise array to store branch ppl per timestep !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    allocate(branch_ppl(1:n_steps,1:num_elems))
    branch_ppl(1:n_steps,1:num_elems) = 0.0_dp
    print*, "Allocated unit_ppl & branch_ppl"
...
!!! calculate the compliance of each tissue unit
    ...
    kount = 1 ! initialise first row of unit_ppl
    call update_pleural_pressure(ppl_current,kount) !calculate new pleural pressure
...
!!! solve for a single breath (for time up to endtime)
       kount=1 ! (MS) initialise timestep kount for that breath
...
!!!.......update the estimate of pleural pressure and radius 
          call update_pleural_pressure(ppl_current,kount) ! new pleural pressure
          call get_elem_ppl(kount) ! Need to get airway plural pressures before update_radius
          call update_radius(compliance,kount)
          call write_flow_step_results(chest_wall_compliance,init_vol, &
...
!!!#############################################################################
  ...
  subroutine update_pleural_pressure(ppl_current,kount)
    ...
    do nunit = 1,num_units
       ...
       u_ppl = unit_field(nu_pe,nunit) + node_field(nj_aw_press,np2) ! (MS)
       unit_ppl(kount,nunit) = u_ppl ! store in array
...
!!!#############################################################################
  subroutine get_elem_ppl(kount)

    integer :: ne,np1,np2, np3, radius, num_closest_units,ne_unit,i, nunit, kount
    real(dp) :: del_x, del_y, del_z, d ! Radius and difference/delta between unit and element x,y,z coordiantes 
    character(len=60) :: sub_name
    real(dp) :: ne_ppl, total, nu_ppl, u_ppl, total_ppl
    ...
    do ne = 1,num_elems !Iterate over all the elements in the airway tree
      ...
      ! reset total_ppl around this branch
      total_ppl = 0.0_dp

      do nunit = 1,num_units !Iterate through units
         u_ppl = unit_ppl(kount,nunit) ! get pleural pressure of unit
         
         ne_unit = units(nunit) !Get element that supplies the unit         
         np3 = elem_nodes(2,ne_unit) ! Get unit number 
         ...
         IF (d<= radius) THEN ! Check if that node is within set radius of airway elemnt midpoint 
            total_ppl = total_ppl + u_ppl ! add pleural pressure
            num_closest_units = num_closest_units + 1 ! Keep number of units  within the radius
         ENDIF

      enddo !End unit 
      
      branch_ppl(kount, nunit) = total_ppl/num_closest_units ! get average pleural pressure around the branch
      
      !IF (num_closest_units == 0) THEN
...
subroutine update_radius(compliance,kount)

   real(dp) :: P_transmural, P_elem, ne_ppl
   integer :: ne, np1, np2, kount
   character(len=60) :: sub_name
...
! Calculate the transmural pressure experienced by the element = difference bewtween the pressure inside the element and the plural pressure 
       P_transmural = P_elem - branch_ppl(kount,ne)
